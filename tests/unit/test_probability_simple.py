...
from minesweeper.inference.probability import (
...
def test_rule_b_all_mines():
    ...
    If (0,0) shows 2 and only two unknown neighbors remain, both must be mines.
    """
    board = Board(3, 3, {(0, 1), (1, 0)})  # Mines adjacent to (0,0)

    # Reveal a known "2" tile and clear the third neighbor so only two unknowns remain
    board.reveal(0, 0)
    board.reveal(1, 1)  # Helps mimic inference after neighboring information
    
    probs = compute_local_probabilities(board)
    
    # The remaining unknown neighbors must be mines
    assert (0, 1) in probs, "Cell (0,1) should have probability"
    assert probs[(0, 1)] == 1.0, "Cell (0,1) should be a mine (prob=1)"

    assert (1, 0) in probs, "Cell (1,0) should have probability"
    assert probs[(1, 0)] == 1.0, "Cell (1,0) should be a mine (prob=1)"
...
def test_rule_c_local_probability():
    ...
    If (0,0) shows 1 and has 3 unknown neighbors, each gets probability 1/3.
    """
    board = Board(3, 3, {(0, 1)})  # Mine at (0,1)
    ...
    # (0,0) has three unknown neighbors, so each gets probability 1/3
    expected_cells = {(0, 1), (1, 0), (1, 1)}
    for cell in expected_cells:
        assert cell in probs, f"Cell {cell} should have probability assigned"
        assert abs(probs[cell] - (1 / 3)) < 0.01, (
            f"Cell {cell} should have prob ~0.33, got {probs[cell]}"
        )
