...
def compute_global_probability(board: Board) -> float:
    ...
    # Count flagged cells (assumed to be mines)
    flagged_count = sum(1 for i in range(board.rows) for j in range(board.cols) 
                       if board.is_flagged(i, j))
    
    # Estimate remaining mines
    # Note: This is an approximation since we don't track total mines
    # In a real game, we'd have: remaining_mines = total_mines - flagged_count
    # For now, we use a heuristic: assume some percentage of unknown cells are mines
    # This is Rule D fallback
    
    # Simple heuristic: if we have constraints, use them
    # Otherwise, use a conservative estimate
    total_cells = board.rows * board.cols
    revealed_count = len(board.revealed_cells())
    unknown_count = len(unknown)
    
    # Estimate: assume mines are roughly 10-20% of board (common in Minesweeper)
    # But this is just a fallback - local constraints should dominate
    estimated_mine_density = 0.15  # 15% of cells are mines (typical)
    estimated_total_mines = int(total_cells * estimated_mine_density)
    remaining_mines = max(0, estimated_total_mines - flagged_count)
